...
export class Client extends EventEmitter {
  ...
  public async getClans(
    query: ClanSearchOptions | string[],
    options?: OverrideOptions
  ) {
    if (Array.isArray(query)) {
      return (
        await Promise.allSettled(query.map((tag) => this.getClan(tag, options)))
      )
  ...
  public async getClanWar(clanTag: string, options?: OverrideOptions) {
    const { data, maxAge, path, status } = await this.rest.getCurrentWar(
      clanTag,
      options
    );
   ...
   * ```
   */
  public async getCurrentWar(
    clanTag: string | { clanTag: string; round?: keyof typeof CWL_ROUNDS },
    options?: OverrideOptions
  ) {
    const args =
      typeof clanTag === "string"
        ? { clanTag }
        : { clanTag: clanTag.clanTag, round: clanTag.round };

    try {
      return await this.getClanWar(args.clanTag, options);
    } catch (e) {
      if (e instanceof HTTPError && [200, 403].includes(e.status)) {
        return this.getLeagueWar(
          { clanTag: args.clanTag, round: args.round },
          options
        );
      }
      throw e;
  ...
  public async getLeagueWar(
    ...
    const wars = await this.util.allSettled(
      warTags.map((warTag) =>
        this.getClanWarLeagueRound(
          { warTag, clanTag: args.clanTag },
          { ...options, ignoreRateLimit: true }
        )
      )
    );
    ...
    if (args.round && args.round in CWL_ROUNDS) {
      return (
        wars.find(
          (war) => war.clan.tag === args.clanTag && war.state === state
        ) ?? null
      );
    }
  ...
  public async getClanWarLeagueGroup(
    clanTag: string,
    options?: OverrideOptions
  ) {
    const { data, status, path, maxAge } =
      await this.rest.getClanWarLeagueGroup(clanTag, options);
    if (data.state === "notInWar") {
      throw new HTTPError(NotInWarError, status, path, maxAge);
    }
  ...
  public async getClanWarLeagueRound(
    ...
    const args =
      typeof warTag === "string"
        ? { warTag }
        : { warTag: warTag.warTag, clanTag: warTag.clanTag };
    const { data, maxAge, status, path } =
      await this.rest.getClanWarLeagueRound(args.warTag, options);
    if (data.state === "notInWar") {
      throw new HTTPError(NotInWarError, status, path, maxAge);
    }
    return new ClanWar(this, data, {
      warTag: args.warTag,
      clanTag: args.clanTag,
      maxAge,
    });
  }
  ...
  public async getPlayers(playerTags: string[], options?: OverrideOptions) {
    return (
      await Promise.allSettled(
        playerTags.map((tag) => this.getPlayer(tag, options))
      )
    )
      .filter((res) => res.status === "fulfilled")
  ...
  public async verifyPlayerToken(
    playerTag: string,
    token: string,
    options?: OverrideOptions
  ) {
    const { data } = await this.rest.verifyPlayerToken(
      playerTag,
      token,
      options
    );
  ...
  public async getLeagueSeasons(options?: SearchOptions) {
    const { data } = await this.rest.getLeagueSeasons(
      LEGEND_LEAGUE_ID,
      options
    );
  ...
  public async getSeasonRankings(seasonId: string, options?: SearchOptions) {
    const { data } = await this.rest.getSeasonRankings(
      LEGEND_LEAGUE_ID,
      seasonId,
      options
    );
   ...
   * For global ranking, use `global` as `locationId`.
   */
  public async getVersusClanRanks(
    locationId: number | "global",
    options?: SearchOptions
  ) {
   ...
   * For global ranking, use `global` as `locationId`.
   */
  public async getVersusPlayerRanks(
    locationId: number | "global",
    options?: SearchOptions
  ) {
  ...
  public on<K extends keyof ClientEvents>(
    event: K,
    listeners: (...args: ClientEvents[K]) => void
  ): this;
  /** @internal */
  ...
  /** @internal */
  public emit<S extends keyof CustomEvents>(
    event: Exclude<S, keyof ClientEvents>,
    ...args: CustomEvents[S]
  ): this;
  /** @internal */ // @ts-expect-error
